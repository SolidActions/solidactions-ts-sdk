...
const doSomethingTraced = SolidActions.registerWorkflow(doSomethingTraced_internal);
...
describe('trace spans propagate ', () => {
  beforeAll(async () => {
    ...
    SolidActions.setConfig({ ...config, enableOTLP: true });
  ...
  test('from-outside-into-SolidActions-calls', async () => {
