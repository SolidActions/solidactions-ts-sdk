import {
  SolidActionsJSON,
  SolidActionsJSONLegacy,
  SERIALIZER_MARKER_KEY,
  SERIALIZER_MARKER_VALUE,
} from '../src/serialization';

/**
 * SolidActionsJSON uses SuperJSON internally for rich type support.
 *
 * Test structure:
 ...
 */
describe('solidactions-json-reviver-replacer', () => {
  test('Replace revive dates', () => {
    ...
    const stringified = SolidActionsJSON.stringify(obj);
    const parsed = SolidActionsJSON.parse(stringified) as typeof obj;
...
describe('SuperJSON enhanced types', () => {
  test('serializes Sets', () => {
    ...
    const serialized = SolidActionsJSON.stringify(set);
    const deserialized = SolidActionsJSON.parse(serialized);
  ...
  test('serializes Maps', () => {
    ...
    const serialized = SolidActionsJSON.stringify(map);
    const deserialized = SolidActionsJSON.parse(serialized);
  ...
  test('preserves undefined values', () => {
    ...
    const serialized = SolidActionsJSON.stringify(obj);
    const deserialized = SolidActionsJSON.parse(serialized) as typeof obj;
  ...
  test('serializes RegExp patterns', () => {
    ...
    const serialized = SolidActionsJSON.stringify(regex);
    const deserialized = SolidActionsJSON.parse(serialized) as RegExp;
  ...
  test('handles circular references', () => {
    ...
    const serialized = SolidActionsJSON.stringify(obj);
    const deserialized = SolidActionsJSON.parse(serialized) as Obj;
  ...
  test('complex nested structures with mixed types', () => {
    ...
    const serialized = SolidActionsJSON.stringify(complex);
    const deserialized = SolidActionsJSON.parse(serialized) as typeof complex;
...
describe('Backwards compatibility', () => {
   ...
   *
   * Context: Before this PR, SolidActionsJSON used custom replacer/reviver functions that created
   * formats like {"dbos_type": "dbos_Date", "dbos_data": "2024-01-01T00:00:00.000Z"}.
   *
   * After this PR: SolidActionsJSON creates SuperJSON format like {"json": {...}, "meta": {...}}.
   *
   * The problem: Millions of workflow states in production were serialized with the OLD format.
   * The solution: SolidActionsJSON.parse() now detects which format and uses the appropriate deserializer.
   *
   * These tests verify that solution works by:
   * 1. Using SolidActionsJSONLegacy (the old implementation) to create old-format strings
   * 2. Parsing those strings with the NEW SolidActionsJSON
  ...
  test('parses legacy SolidActionsJSON dates', () => {
    const date = new Date('2024-01-01T12:00:00Z');
    const legacySerialized = SolidActionsJSONLegacy.stringify(date);
    const deserialized = SolidActionsJSON.parse(legacySerialized);
    expect(deserialized).toEqual(date);
  });

  test('parses legacy SolidActionsJSON BigInts', () => {
    const bigint = BigInt(123456789);
    const legacySerialized = SolidActionsJSONLegacy.stringify(bigint);
    const deserialized = SolidActionsJSON.parse(legacySerialized);
    expect(deserialized).toEqual(bigint);
  });

  test('parses legacy SolidActionsJSON complex objects', () => {
    const obj = {
      date: new Date('2024-01-01'),
      bigint: BigInt(42),
    ...
    const legacySerialized = SolidActionsJSONLegacy.stringify(obj);
    const deserialized = SolidActionsJSON.parse(legacySerialized);
    expect(deserialized).toEqual(obj);
  });

  test('new SolidActionsJSON can parse old database data', () => {
    // Simulate data that was stored in DB with old SolidActionsJSON
    const originalData = {
      id: 'test-123',
    ...
    const dbStored = SolidActionsJSONLegacy.stringify(originalData);

    // New SolidActionsJSON should be able to parse it
    const parsed = SolidActionsJSON.parse(dbStored);
    expect(parsed).toEqual(originalData);
  });

  test('handles null correctly', () => {
    // These are legacy cases
    expect(SolidActionsJSON.parse(null)).toBe(null);
    expect(SolidActionsJSON.parse(undefined)).toBe(null);

    expect(SolidActionsJSON.parse(SolidActionsJSON.stringify(null))).toBeNull();
    expect(SolidActionsJSON.parse(SolidActionsJSON.stringify(undefined))).toBeUndefined();
    expect(SolidActionsJSON.stringify(null)).toBeTruthy();
    expect(SolidActionsJSON.stringify(undefined)).toBeTruthy();
  });

  test('parses plain JSON', () => {
    const plain = { simple: 'object', number: 42, bool: true };
    const plainSerialized = JSON.stringify(plain);
    const deserialized = SolidActionsJSON.parse(plainSerialized);
  ...
  test('does not confuse user data with SuperJSON format', () => {
    ...
    const ambiguousDeserialized = SolidActionsJSON.parse(ambiguousSerialized);
    ...
    const moreDeserialized = SolidActionsJSON.parse(moreSerialized);
    expect(moreDeserialized).toEqual(moreAmbiguous);
  });

  test('new SolidActionsJSON always includes serializer marker to avoid ambiguity', () => {
    // Simple values should get our marker
    const simpleValue = { foo: 'bar' };
    const serialized = SolidActionsJSON.stringify(simpleValue);
    ...
    const complexSerialized = SolidActionsJSON.stringify(complexValue);
