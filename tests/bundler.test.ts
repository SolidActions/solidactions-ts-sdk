...
import { generateSolidActionsTestConfig, setUpSolidActionsTestServer } from './helpers';
import { SolidActions } from '../src';
import { Client } from 'pg';

describe('SolidActions Bundler Tests', () => {
  ...
  test('test app with bundler', async () => {
    ...
    expect(bundleContent).toContain('SolidActions');
    ...
    const runPromise2 = new Promise<number>((resolve, reject) => {
      const child = spawn('node', [bundleFile], {
        ...
        env: {
          ...
          SOLIDACTIONS_DATABASE_URL: `postgresql://postgres:${dbPassword}@localhost:5432/${testDbName}`,
        },
      });
