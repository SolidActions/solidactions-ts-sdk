...
type SolidActionsSerializeType = 'solidactions_Date' | 'solidactions_BigInt';

interface SolidActionsSerialized {
  solidactions_type: SolidActionsSerializeType;
}

interface SolidActionsSerializedDate extends SolidActionsSerialized {
  solidactions_type: 'solidactions_Date';
  solidactions_data: string;
}

interface SolidActionsSerializedBigInt extends SolidActionsSerialized {
...
export function SolidActionsReplacer(this: any, key: string, value: unknown) {
  // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-assignment
  const actualValue = this[key];
  if (actualValue instanceof Date) {
    const res: SolidActionsSerializedDate = {
      solidactions_type: 'solidactions_Date',
  ...
  if (typeof actualValue === 'bigint') {
    const res: SolidActionsSerializedBigInt = {
      solidactions_type: 'solidactions_BigInt',
...
function isSerializedDate(value: unknown): value is SolidActionsSerializedDate {
  return (
    typeof value === 'object' &&
    value !== null &&
    (value as Record<string, unknown>).solidactions_type === 'solidactions_Date'
  );
}

function isSerializedBigInt(value: unknown): value is SolidActionsSerializedBigInt {
  return (
    typeof value === 'object' &&
    value !== null &&
    (value as Record<string, unknown>).solidactions_type === 'solidactions_BigInt'
  );
}

export function SolidActionsReviver(_key: string, value: unknown): unknown {
  switch (true) {
...
export const SolidActionsJSONLegacy = {
...
export const SERIALIZER_MARKER_KEY = '__solidactions_serializer';
...
type SolidActionsBrandedSuperjsonRecord = SuperJSONResult & {
 ...
 */
function isSolidActionsBrandedSuperjsonRecord(obj: unknown): obj is SolidActionsBrandedSuperjsonRecord {
  return (
    typeof obj === 'object' &&
    obj !== null &&
 ...
 */
export const SolidActionsJSON: SolidActionsSerializer = {
  parse: (text: string | null | undefined): unknown => {
    ...
    return SolidActionsJSONLegacy.parse(text);
  },
...
export function serializeFunctionInputOutput<T>(
  ...
  if (serializer === SolidActionsJSON && isObjectish(deserialized)) {
...
export function safeParse(serializer: SolidActionsSerializer, val: string) {
  try {
...
export function safeParseError(serializer: SolidActionsSerializer, val: string) {
  try {
