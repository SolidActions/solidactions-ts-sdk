 ...
 *
 * Returns `{ vars: [], connections: [] }` for any yaml that has no `env:` key
 * or an empty/non-array `env:` value — safe to call on any yaml without
 * crashing or throwing (except for invalid yaml syntax, which is bubbled up).
 */
export function parseDeclarations(yamlContent: string): VarsDeclarations {
  ...
  for (const item of env) {
    if (typeof item === 'string') {
      ...
      // Shape: `- VAR_NAME: GLOBAL_NAME` or `- VAR_NAME: { oauth: "..." }`
      // In yaml, these parse to objects with exactly one key: the var name.
