...
type ContextualMetadata = {
  ...
  span?: SolidActionsSpan; // All context metadata should be attributes of the context's span
};
...
export class GlobalLogger {
  ...
  info(logEntry: unknown, metadata?: ContextualMetadata): void {
    ...
    if (typeof logEntry === 'string') {
      ...
      this.logger.info(SolidActionsJSON.stringify(logEntry), metadata);
    }
  ...
  debug(logEntry: unknown, metadata?: ContextualMetadata): void {
    ...
    if (typeof logEntry === 'string') {
      ...
      this.logger.debug(SolidActionsJSON.stringify(logEntry), metadata);
    }
    this.isLogging = false;
  }
  ...
  warn(logEntry: unknown, metadata?: ContextualMetadata): void {
    ...
    if (typeof logEntry === 'string') {
      ...
      this.logger.warn(SolidActionsJSON.stringify(logEntry), metadata);
    }
    this.isLogging = false;
  }
  ...
  error(inputError: unknown, metadata?: ContextualMetadata & StackTrace): void {
    ...
    if (inputError instanceof Error) {
      ...
      this.logger.error(SolidActionsJSON.stringify(inputError), { ...metadata, stack: new Error().stack });
    }
    this.isLogging = false;
  }
...
export class SolidActionsContextualLogger implements DLogger {
  readonly includeContextMetadata: boolean;
  constructor(
    private readonly globalLogger: GlobalLogger,
    readonly ctx: () => SolidActionsSpan | undefined,
  ) {
...
export class SolidActionsConsoleLogger implements DLogger {
...
function getConsoleFormat() {
  ...
  return format.combine(
    ...
    format.printf((info) => {
      ...
      const messageString: string = typeof message === 'string' ? message : SolidActionsJSON.stringify(message);
      const fullMessageString = `${messageString}${info.includeContextMetadata ? ` ${SolidActionsJSON.stringify((info.span as SolidActionsSpan)?.attributes)}` : ''}`;
