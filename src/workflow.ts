...
import { SolidActions, runInternalStep } from './solidactions';
...
export class InvokedHandle<R> implements InternalWFHandle<R> {
  ...
  async getStatus(): Promise<WorkflowStatus | null> {
    return await SolidActions.getWorkflowStatus(this.workflowUUID);
  }
...
export class RetrievedHandle<R> implements InternalWFHandle<R> {
  ...
  async getResult(funcIdForGet?: number): Promise<R> {
    return (await SolidActions.getResultInternal<R>(
      this.workflowUUID,
      undefined,
      undefined,
      funcIdForGet,
...
registerSerializationRecipe<WorkflowHandle<unknown>, { wfid: string }>({
  ...
  deserialize: (s: { wfid: string }) =>
    new RetrievedHandle(SolidActionsExecutor.globalInstance!.systemDatabase, s.wfid),
});
