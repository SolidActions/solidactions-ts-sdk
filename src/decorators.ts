...
import { SolidActionsConflictingRegistrationError, SolidActionsNotRegisteredError } from './error';
...
/**
 * Interface for naming SolidActions-registered functions.
...
/**
 * Interface for integrating into the SolidActions startup/shutdown lifecycle
 */
export interface SolidActionsLifecycleCallback {
  /** Called back during SolidActions.launch */
...
export interface SolidActionsMethodMiddlewareInstaller {
 ...
 */
export type SolidActionsFieldType =
  | 'integer'
  | 'double'
  | 'decimal'
  | 'timestamp'
...
export class SolidActionsDataType {
  dataType: SolidActionsFieldType = 'text';
  length: number = -1;
  precision: number = -1;
  scale: number = -1;
  ...
  static fromArg(arg?: Function): SolidActionsDataType | undefined {
...
export interface ArgDataType {
  dataType?: SolidActionsDataType; // Also a very simplistic data type format... for native scalars or JSON
}
...
export const SOLIDACTIONS_AUTH = 'auth';
...
export interface MethodRegistrationBase {
  ...
  // eslint-disable-next-line @typescript-eslint/no-unsafe-function-type
  registeredFunction: Function | undefined; // Function that is called by SolidActions engine, including input validation and role check
  // eslint-disable-next-line @typescript-eslint/no-unsafe-function-type
...
export class MethodRegistration<This, Args extends unknown[], Return> implements MethodRegistrationBase {
  ...
  checkFuncTypeUnassigned(newType: string) {
    ...
    if (error) {
      ...
      throw new SolidActionsConflictingRegistrationError(`${error}`);
    } else {
  ...
  getRequiredRoles() {
    const rr = this.getRegisteredInfo(SOLIDACTIONS_AUTH) as MethodAuth;

    if (rr?.requiredRole) {
      return rr.requiredRole;
    }

    const drr = this.defaults?.getRegisteredInfo(SOLIDACTIONS_AUTH) as ClassAuthDefaults;
...
export abstract class ConfiguredInstance {
  ...
  constructor(name: string) {
    if (solidActionsLaunchPoint) {
      console.warn(
        `ConfiguredInstance '${name}' is being created after SolidActions initialization and was not available for recovery.`,
      );
    }
    this.name = name;
...
let solidActionsLaunchPoint: string[] | undefined = undefined;
export function recordSolidActionsLaunch() {
  solidActionsLaunchPoint = new StackGrabber().getCleanStack(2); // Remove one for record, one for registerAndWrap...
}
export function recordSolidActionsShutdown() {
  solidActionsLaunchPoint = undefined;
}

export function ensureSolidActionsIsNotLaunched() {
  if (solidActionsLaunchPoint) {
    throw new SolidActionsConflictingRegistrationError(
      `SolidActions code is being registered after SolidActions.launch().  SolidActions was launched from:\n${solidActionsLaunchPoint.join('\n')}\n`,
    );
  }
}

export function ensureSolidActionsIsLaunched(reason: string) {
  if (!solidActionsLaunchPoint) {
...
const lifecycleListeners: SolidActionsLifecycleCallback[] = [];
export function registerLifecycleCallback(lcl: SolidActionsLifecycleCallback) {
  if (!lifecycleListeners.includes(lcl)) lifecycleListeners.push(lcl);
}
export function getLifecycleListeners() {
  return lifecycleListeners as readonly SolidActionsLifecycleCallback[];
}

// Middleware installers - insert middleware in registered functions prior to launch
let installedMiddleware = false;
const middlewareInstallers: SolidActionsMethodMiddlewareInstaller[] = [];

export function registerMiddlewareInstaller(i: SolidActionsMethodMiddlewareInstaller) {
...
export function wrapSolidActionsFunctionAndRegisterByTarget<This, Args extends unknown[], Return>(
  target: object,
  propertyKey: PropertyKey,
  name: string | undefined,
...
export function wrapSolidActionsFunctionAndRegisterByUniqueName<This, Args extends unknown[], Return>(
...
export function wrapSolidActionsFunctionAndRegisterDec<This, Args extends unknown[], Return>(
  target: object,
  propertyKey: PropertyKey,
  name: string | undefined,
  descriptor: TypedPropertyDescriptor<(this: This, ...args: Args) => Promise<Return>>,
...
export function wrapSolidActionsFunctionAndRegister<This, Args extends unknown[], Return>(
  ctorOrProto: object | undefined,
  className: string | undefined,
  propertyKey: PropertyKey,
  name: string | undefined,
...
export function getClassRegistrationByName(name: string, create: boolean = false) {
  if (!classesByName.has(name) && !create) {
    throw new SolidActionsNotRegisteredError(name, `Class '${name}' is not registered`);
  }
